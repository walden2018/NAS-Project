...
class Optimizer:
    ...
    def __init__(self, dimension, ls):
    ...
    def follow_constraint(self, x):

        for i in range(len(self.__constraint)):
            prob_sum = 0.0
            for j in range(len(self.__constraint[i])):
                prob_sum += x[self.__constraint[i][j]]
            if prob_sum > 1.0:
                return False
